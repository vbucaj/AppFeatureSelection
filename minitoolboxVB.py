...
class FeatureSelection(object):
    ...
    def feature_selection(self,predictors=None,target=None,boxPlot=True,roc=True,
       ...
       if regModel==True and boxPlot==False:
       ...
       elif regModel==True and boxPlot==True and self.r2_score==True:
            ...
            fig.update_layout(template="plotly_dark", width=self.length, height=self.height,
                              title_text="Feature Selection",
                              xaxis_title='Features',
                              yaxis_title='Percentage decrease in {}'.format('R2 Score')
                              )

            mark_color=cycle(['red', 'green', 'purple', 'yellow', 'lightgreen','indianred', 'MediumPurple','LightSkyBlue'])
            for feature,color in zip(df_model.columns,mark_color):
                c=color
                fig.add_trace(go.Box(y=df_model[feature], name=feature,marker_color=c))

            st.plotly_chart(fig)

       elif regModel == True and boxPlot == True and self.mse == True:
            ...
            fig.update_layout(template="plotly_dark", width=self.length, height=self.height,
                              title_text="Feature Selection",
                              xaxis_title='Features',
                              yaxis_title='Percentage increase in {}'.format('MSE')
                              )


            # mark_color = cycler('color', ['red', 'green', 'purple', 'yellow', 'lightgreen',
            #                               'indianred', 'MediumPurple','LightSkyBlue'])
            mark_color=cycle(['red', 'green', 'purple', 'yellow', 'lightgreen','indianred', 'MediumPurple','LightSkyBlue'])
            for feature, color in zip(df_model.columns, mark_color):
                c = color
                fig.add_trace(go.Box(y=df_model[feature], name=feature, marker_color=c))
